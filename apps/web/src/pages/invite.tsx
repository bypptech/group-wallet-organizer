...
export default function InvitePage() {
  ...
  // URLからトークンとvaultIdを取得
  // 2つの形式をサポート:
  // 1. /invite?code=xxx&vault=yyy
  // 2. /invite/:token
  const params = new URLSearchParams(window.location.search);
  const pathParts = window.location.pathname.split('/').filter(Boolean);
  const inviteCode = params.get('code') || (pathParts.length > 1 ? pathParts[1] : null);
  ...
  const fetchInviteDetails = async () => {
    try {
      ...
      // Mock mode: localStorageから取得を試みる
      const storedInvites = Object.keys(localStorage)
        .filter(key => key.startsWith('invite:'))
        .map(key => JSON.parse(localStorage.getItem(key) || '{}'));

      const localInvite = storedInvites.find(inv => inv.token === inviteCode || inv.inviteCode === inviteCode);

      if (localInvite) {
        setInvite({
          id: localInvite.id || 'local-invite',
          token: inviteCode!,
          vaultId: localInvite.vaultId || vaultId || '',
          role: localInvite.role,
          weight: localInvite.weight || 1,
          expiresAt: localInvite.expiresAt,
          createdBy: localInvite.createdBy,
          usedAt: localInvite.usedAt,
          usedBy: localInvite.usedBy,
        });
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md">
        ...
        <CardContent className="space-y-4">
          ...
          {/* Shareable Key Permissions Display */}
          {invite.metadata && typeof invite.metadata === 'object' && 'source' in invite.metadata && invite.metadata.source === 'shareable-key' && (
            <div className="mt-4 p-4 bg-blue-500/10 border border-blue-500/20 rounded-md">
              <div className="flex items-center gap-2 mb-2">
                <Shield className="h-4 w-4 text-blue-400" />
                <p className="text-sm font-medium text-blue-400">
                  Shareable Key Access Permissions
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                {(invite.metadata as any).permissions?.map((perm: string) => (
                  <Badge key={perm} variant="outline" className="text-blue-300 border-blue-500/30 bg-blue-500/10">
                    {perm}
                  </Badge>
                ))}
              </div>
              <p className="text-xs text-muted-foreground mt-2">
                Key Type: {((invite.metadata as any).keyType || 'vault').toUpperCase()}
              </p>
            </div>
          )}
