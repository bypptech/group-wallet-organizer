...
- 5. AI共通設定の正本: `.ai/{config.yaml, quality-gates.yaml}`（各ツール側は生成物）
- 6. ドラフト仕様: `docs/{specs,design,tasks}`（レビュー後 `.kiro/specs` に同期）

補足:
- 「何を作るか」は `.kiro/specs`（要件/設計/タスク）を優先。
- 「どう進めるか」は本 AGENTS.md の運用規範を優先。
- `.ai/` は AI 設定の単一ソース。`.claude/` `.codex/` `.gemini/` `.kiro/` は生成物・参照物として扱う。
...
- データ: Drizzle ORM（PostgreSQL 推奨 / Supabase 互換）
...
AI設定の単一ソースと同期:
- 設定/生成: `npm run ai:migrate`
- コマンド雛形配布（Claude/Codex）: `npm run agents:sync`
- MCP一覧確認: `npm run mcp:check`

仕様書の運用（重複回避）:
- ドラフト（docs）→ 正本（.kiro/specs）へ同期: `npm run specs:sync -- --project <name>`
- 逆方向（必要時のみ）: `npm run specs:sync -- --project <name> --from kiro`

コマンド表記の統一:
- 仕様/TDD/逆生成のエージェントコマンドは `@kairo-*`, `@tdd-*`, `@rev-*` の表記に統一する。
...
本リポジトリではデフォルトのフロントは「React + Vite」、バックエンドは「Hono」です。以下のサンプルには Next.js など別スタックの例が含まれますが、構成・命名は現行スタックに読み替えて適用してください（例: `src/app` の代わりに `apps/web/src` など）。
